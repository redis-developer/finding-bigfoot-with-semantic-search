...
This will launch four containers:
  ...
  1. An instance of Redis Stack that's preloaded with a bunch of Bigfoot sightings and exposed on port 6379.
...
Now we need to install the packages that the API and the web interface need. For the API, run the following in the `api` folder:

```bash
npm install
```

And likewise, for the web interface, run this in the `web` folder:
...
The rest is easy as it's just another Node.js application. So, from the `embedder` folder install all the packages:
